...
# from origianl blackhawk bashrc
# Read the system wide bashrc.
...
# add to front of path so my local programs overide system
export PATH=$HOME/local/bin:$PATH

# Add ~/local folder to man path
...
# config command for dotfile configs https://www.atlassian.com/git/tutorials/dotfiles
...
alias cp='cp -i'
alias mv='mv -i'
#alias rm='rm -iv' # I am an adult, I don't need hand holding
alias mkdir='mkdir -p'
alias ps='ps auxf'
...
function __setprompt
{
	...
	#PS1+="${BLUE} $(date +'%-I':%M:%S%P)\[${LIGHTGRAY}\])-" # Time
...
{(  # use a subshell so it doesn't kill terminal when error
    ...
    subdir=$HOME/cpe212/daily

    #echo "creating dir $subdir"
    #mkdir -v $subdir
    echo "unzipping $basename zip_file into $subdir"
    unzip $zip_file -d $subdir
    rm -rf $subdir/__MACOSX
    #echo "removing original file: $zip_file"
    #rm $zip_file
    #make a folder for other stuff
    daily_folder=$subdir/$(date +%d_%b_%a)
    echo "Making folder $daily_folder"
    mkdir $daily_folder
    cd $subdir/$basename
